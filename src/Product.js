...
const Product = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://backend-iuq5.vercel.app/api/product/${id}`);
      fetchProducts();
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.put(`https://backend-iuq5.vercel.app/api/update/${editedProduct['S.no']}`, updatedProduct);
